...
function Select({ items, children, ...props }: SelectProps) {
  const derivedItems = React.useMemo(
    () => (items ? items : deriveItems(children)),
    [children, items],
  );
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "border-input data-placeholder:text-muted-foreground bg-transparent dark:bg-input/30 dark:hover:bg-input/50 focus-visible:border-ring focus-visible:ring-ring/30 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive dark:aria-invalid:border-destructive/50 gap-1.5 rounded-md border px-2 py-1.5 text-xs/relaxed transition-colors focus-visible:ring-[2px] aria-invalid:ring-[2px] data-[size=default]:h-7 data-[size=sm]:h-6 *:data-[slot=select-value]:flex *:data-[slot=select-value]:gap-1.5 [&_svg:not([class*='size-'])]:size-3.5 flex w-fit items-center justify-between whitespace-nowrap outline-none disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
      {...props}
    >
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.GroupLabel
      ...
      className={cn(
        'text-muted-foreground sticky top-0 z-10 -mx-1 px-2 py-1.5 text-xs bg-transparent border-b border-border/50',
        className,
      )}
      {...props}
    />
