...
export default function BagRatesPage() {
  ...
  const [cropYearStartYear, setCropYearStartYear] = React.useState<
    number | null
  >(null);
  ...
  React.useEffect(() => {
    ...
    for (const item of bagRateItems) {
      ...
      base[item.riceType.code].KG_40 =
        typeof item.rates.KG_40 === 'number'
          ? truncateToTwoDecimals(item.rates.KG_40)
          : '';
      base[item.riceType.code].KG_75 =
        typeof item.rates.KG_75 === 'number'
          ? truncateToTwoDecimals(item.rates.KG_75)
          : '';
      base[item.riceType.code].KG_100 =
        typeof item.rates.KG_100 === 'number'
          ? truncateToTwoDecimals(item.rates.KG_100)
          : '';
    }
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      for (const rt of riceTypes) {
        ...
        if (n100 === null) {
          throw new Error(
            `Enter a rate for ${rt.code} (${formatBagSizeLabel('KG_100')}).`,
          );
        }
        if (n75 === null) {
          throw new Error(
            `Enter a rate for ${rt.code} (${formatBagSizeLabel('KG_75')}).`,
          );
        }
        if (n40 === null) {
          throw new Error(
            `Enter a rate for ${rt.code} (${formatBagSizeLabel('KG_40')}).`,
          );
        }

        if (n100 < 0) {
          throw new Error(
            `Enter a valid rate for ${rt.code} (${formatBagSizeLabel('KG_100')}).`,
          );
        }
        if (n75 < 0) {
          throw new Error(
            `Enter a valid rate for ${rt.code} (${formatBagSizeLabel('KG_75')}).`,
          );
        }
        if (n40 < 0) {
          throw new Error(
            `Enter a valid rate for ${rt.code} (${formatBagSizeLabel('KG_40')}).`,
          );
        }
    ...
    <Card>
      <CardHeader>
        ...
        <div className="text-sm text-muted-foreground">
          Set bag rates for each rice type and bag size. Rates are stored in
          rupees.
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="grid gap-4 md:grid-cols-2">
          <Field>
            ...
            <Select
              value={
                typeof selectedYear === 'number' ? String(selectedYear) : ''
              }
          ...
          <Field>
            ...
            <Select
              value={seasonCode}
              onValueChange={(v) =>
                setSeasonCode((v ?? 'KHARIF') as SeasonCode)
              }
            >
              <SelectTrigger className="w-full">
                <SelectValue />
        ...
        <FieldDescription>
          Enter the 100kg rate. The 75kg and 40kg rates will be calculated
          automatically.
        </FieldDescription>
                ...
                <TableRow key={rt.code}>
                      ...
                      <InputGroupInput
                        ...
                        onChange={(e) => {
                          ...
                          setRates((prev) => {
                            ...
                            if (num !== null) {
                              newRates.KG_75 = truncateToTwoDecimals(
                                (num / 100) * 75,
                              );
                              newRates.KG_40 = truncateToTwoDecimals(
                                (num / 100) * 40,
                              );
                            } else {
                              newRates.KG_75 = '';
                  ...
                  <TableCell>
                    <InputGroup className="bg-muted/40 opacity-80 border-muted-foreground/10 cursor-not-allowed">
                      <InputGroupAddon className="bg-transparent opacity-50">
                        ₹
                      </InputGroupAddon>
                      <InputGroupInput
                        readOnly
