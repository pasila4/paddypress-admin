...
export async function listAdminIkpCenters(
  params: {
    page?: number;
    limit?: number;
    search?: string;
  ...
  if (params.stateId && params.stateId.trim() !== '')
    searchParams.set('stateId', params.stateId.trim());
  ...
  if (params.mandal && params.mandal.trim() !== '')
    searchParams.set('mandal', params.mandal.trim());
  if (params.mandalId && params.mandalId.trim() !== '')
    searchParams.set('mandalId', params.mandalId.trim());
  if (params.village && params.village.trim() !== '')
    searchParams.set('village', params.village.trim());
  if (params.villageId && params.villageId.trim() !== '')
    searchParams.set('villageId', params.villageId.trim());
...
export async function createAdminIkpCenter(
  payload: CreateAdminIkpCenterRequest,
) {
  const validated = CreateAdminIkpCenterRequestSchema.safeParse(payload);
  if (!validated.success)
    throw new Error('Check the center details and try again.');
...
export async function deleteAdminIkpCenterPermanently(id: string) {
  const res = await apiFetch(
    `/admin/ikp-centers/${encodeURIComponent(id)}/permanent`,
    {
      method: 'DELETE',
    },
  );
...
export async function updateAdminIkpCenter(
  id: string,
  payload: UpdateAdminIkpCenterRequest,
) {
