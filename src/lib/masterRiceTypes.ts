...
export async function listMasterRiceTypes(params: {
  search?: string;
...
export async function upsertMasterRiceType(code: string, payload: UpsertMasterRiceTypeRequest) {
  const normalizedCode = code.trim().toUpperCase();
  if (!normalizedCode) {
    throw new Error("Enter a rice type code.");
  }

  const res = await apiFetch(`/admin/rice-types/${encodeURIComponent(normalizedCode)}`, {
    method: "PUT",
